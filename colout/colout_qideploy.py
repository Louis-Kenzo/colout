...
def theme():
    ...
    return [
        # qideploy config
        [ "^(Using profile:) (.*)$", ",".join([section_label,section_title]), "normal,bold" ],
        [ "^The following projects$", section_label, "normal" ],
        [ "^ (\*) ([\w\-]+)$", ",".join([section_label,section_title]), "normal,bold" ],
        [ "^will be deployed to", section_label, "normal" ],
        [ "^::  Deploying projects", section_label, "reverse" ],
        # qideploy progress
        [ "^(\* \( *[0-9]+/[0-9]+\)) (Deploying project) (.*) (to) ", ",".join(["red", section_label, section_title]), "bold,bold,bold" ],
        ...
        [ "^([^\s]+/)(.*)$", ",".join([neutral, info]), "normal,bold" ],
        [ "^\s+\d+\s+\d{1,3}%\s+(\d{1,3}\.\d{2}\wB/s)\s+\d{1,2}:\d{2}:\d{2}\s+\(xfer#\d{1,4}, to-check=(\d+/\d+)\)$", ",".join([neutral,"Scale"]), "bold" ],
